...
- Astra's automatic Codex routing uses [codex-delegation](../../codex-delegation/SKILL.md): an interactive Codex TUI in a split pane, without Orca orchestration. For other supervised coordination (task DAGs, dispatches, inbox/reply flows), use `orca orchestration ...` commands; see **Supervised Dispatch**. A receiving agent can run `orca orchestration check --unread --inject` to render its unread mail in agent-readable form; this checks the caller's inbox and does not remotely deliver input to another terminal.
