---
name: "orca-cli"
description: "Public `orca` CLI for Orca-managed state: worktrees, terminals, repos, automations, worktree comments, and the browser embedded inside the Orca app. Use ONLY when the user explicitly mentions Orca (\"orca cli\", \"Orca worktree\", \"Orca terminal\", \"Orca browser\", \"$orca-cli\"), or when the CLAUDE.md \uc6cc\ud06c\ud2b8\ub9ac \ubd84\ub9ac rule routes a worktree to supervised dispatch or to a full handoff. A generic \"hand off to another agent/worktree\" request without an Orca mention and without that routing is not this skill; delegating to codex additionally requires the user to ask for codex (`codex-delegation` skill)."
---
