...
4. Convert surfaces in the same order the CLI uses:
   ...
   - hooks: rewrite supported Claude hooks into `.codex/hooks.json`; hooks are enabled by default, and generated config uses the canonical `[features].hooks = true` key
